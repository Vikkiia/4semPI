...
namespace perestan
{
    permutation::permutation(short n)
    {
        ...
        this->dart = new bool[n];// напрвления стрелок
        this->reset();
    };
    ...
    __int64  permutation::getfirst()
    {
        ...
        {
            this->sset[i] = i; this->dart[i] = L;//заполняем массив индексами и стрелки влево
        };
        return  (this->n > 0) ? this->np : -1;
    };
    __int64  permutation::getnext()   // Ищем максимальный элемент, который можно переместить в его направлени
    {
        __int64 rc = -1;
        ...
        for (int i = 0; i < this->n; i++)
        {
            if (i > 0 &&
                ...
                maxm < this->sset[i])  maxm = this->sset[idx = i];//Меняем направления стрелок (dart[]) для всех элементов, больших maxm.
            if (i < (this->n - 1) &&
                this->dart[i] == R &&
                this->sset[i] > this->sset[i + 1] &&
