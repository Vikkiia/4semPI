...
enum Dart { TOP, LEFT, LEFTTOP };//откуда пришло значение в матрице:

int lcs(int lenx, const char x[],//Рекурсивный метод (lcs)
	int leny, const char y[])
{
	int rc = 0;
	if (lenx > 0 && leny > 0)
	{
		if (x[lenx - 1] == y[leny - 1]) rc = 1 + lcs(lenx - 1, x, leny - 1, y);//Символы совпадают → берём диагональ +1
		else rc = std::max(lcs(lenx, x, leny - 1, y), lcs(lenx - 1, x, leny, y));//Берём максимум из возможных LCS
	}
	return rc;        //длина LCS
}
