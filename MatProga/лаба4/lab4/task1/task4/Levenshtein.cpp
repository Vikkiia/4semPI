...
{
	int rc = 0;//Если одна из строк пустая, расстояние Левенштейна равно длине другой строки.
	if (lx == 0)
		rc = ly;
	else  if (ly == 0)//Если символы совпадают, то расстояние 0 
		rc = lx;
	else  if (lx == 1 && ly == 1 && x[0] == y[0])
		rc = 0;
	else  if (lx == 1 && ly == 1 && x[0] != y[0])//Если разные, то нужно 1 замен	
		rc = 1;
	else
		rc = min3(
			levenshtein_r(lx - 1, x, ly, y) + 1,//// Удаление символа из x
			levenshtein_r(lx, x, ly - 1, y) + 1, // Вставка символа в x
			levenshtein_r(lx - 1, x, ly - 1, y) + (x[lx - 1] == y[ly - 1] ? 0 : 1) // Замена символа   // Если символы равны, замена не нужна
		);
	return rc;
};