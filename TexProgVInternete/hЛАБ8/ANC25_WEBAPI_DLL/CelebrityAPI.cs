...
using System.Linq; // Для LINQ-методов, если они используются
...
namespace ANC25_WEBAPI_DLL
{
    public static class CelebrityAPI
    {
        ...
        {
            // Регистрация IRepository (конкретного, из DAL_Celebrity_MSSQL)
            // Теперь мы используем DAL_Celebrity_MSSQL.Repository
            builder.Services.AddScoped<DAL_Celebrity_MSSQL.IRepository>(sp =>
            {
                ...
                // Используем конструктор Repository(string connectionString)
                return new DAL_Celebrity_MSSQL.Repository(config.ConnectionString);
            });

            // Регистрация CountryCodes
            ...
            // Регистрация CelebrityTitles (если это простой класс без зависимостей)
