namespace DAL_Celebrity_MSSQL
{
    public class Init
    {
        static string connectionString = "Server=localhost\\SERVERVILKI; Database=Lab6_Db; Trusted_Connection=true; TrustServerCertificate=True";


        public Init() { }
        public Init(string connStr) { connectionString = connStr; }//Конструктор, который принимает строку подключения и сохраняет её в статическое поле connectionString.

        public static void Execute(bool delete = true, bool create = true)
        /*Статический метод Execute, который может:

удалить базу данных (если delete = true);

создать базу данных (если create = true);

используется для инициализации (например, в тестах или при первом запуске приложения).
